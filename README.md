...
- 검색 url
    - GET 방식으로 쿼리를 전달하게 되면 url이 명시된 방식과 다르게 설정되는 문제점이 있었습니다. (버튼 클릭시 url에 문자열을 넣어주는 방법을 몰랐습니다. 이 방법은 조금 비효율적인 것 같습니다.)
    - 그래서 검색 버튼 클릭시 get 방식으로 입력된 문자를 view단에 임시로 거쳐가는 view인 PostSearchQuery를 동해서 검색 단어를 얻어 url을 맞춰줄 수 있도록 했습니다.
    - 그런 후에 문자열이 url에 들어갈 수 있도록 redirect를 했습니다.
    ```python
    ...
    ```
    - 템플릿은 post_list.html을 공통으로 사용했습니다.
...
1. LightSail 배포해보기
2. profile 및 회원정보 수정, 내가작성한 게시물 리스트 보기 기능
3. 글작성시 toast ui 넣어보기
4. 이미지 여러개 업로드
5. 글 수정시 현재 올라간 이미지 이름 표시해주기
6. 글 제목/내용/작성자 내용으로 검색기능
7. pagenation
